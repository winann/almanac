...
func XL1_calc(zn int, t float64, n int) float64 { //计算月亮
	var ob = XL1[zn]
	var i, j, N int
	var F []float64
	var v float64
...
func E_Lon(t, n float64) float64 {
	return XL0_calc(0, 0, t, n)
}

//月球经度计算,返回Date分点黄经,传入世纪数,n是项数比例
func M_Lon(t float64, n int) float64 {
	return XL1_calc(0, t, n)
}

// MS_aLon_t2 已知月日视黄经差求时间,高速低精度,误差不超过600秒(只验算了几千年)
...
//计算世界时与原子时之差,传入年
func dt_calc(y float64) float64 {
	var y0 = dt_at[len(dt_at)-2] //表中最后一年
	var t0 = dt_at[len(dt_at)-1] //表中最后一年的deltatT
	if y >= y0 {
...
//月日视黄经的差值
func MS_aLon(t float64, Mn int, Sn float64) float64 {
...
//月球经度光行差,误差0.07"
func gxc_moonLon(t float64) float64 {
	return -3.4e-6
}
